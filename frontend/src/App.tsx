...
function ReportsPage() {
  ...
  async function createSchedule() {
    ...
    const json = await response.json()
    setScheduleMessage(response.ok ? `Campanha criada #${json.id}` : (json.message || 'Erro ao criar campanha'))
  }
  useEffect(() => { loadFilters().catch(() => undefined); loadGroups().catch(() => undefined); loadPeople().catch(() => undefined); loadReportTypes().catch(() => undefined); loadReport(today, '', '', 5).catch(() => undefined) }, [])
    ...
    : scheduleForm.targetType === 'vendedor'
      ...
      : scheduleForm.targetType === 'supervisor'
        ...
        : scheduleForm.targetType === 'gerente'
          ...
          : scheduleForm.targetType === 'all_vendedores'
            ? []
            : []
  ...
  const campaignTargetHint = scheduleForm.scope === 'group'
    ? null
    : scheduleForm.targetType === 'vendedor'
