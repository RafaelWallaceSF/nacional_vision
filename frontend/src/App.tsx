...
type Schedule = { id: number; rule_name: string; report_type_code: string; target_type: string; target_id: string; send_time: string; frequency: string; channel: string; active: boolean; created_at?: string }
type HistoryItem = { id: number; rule_name: string; report_type_code: string; target_type: string; target_id: string; status: string; created_at: string }
...
function DashboardPage() {
  ...
  return <section className="page"><div className="page-header"><div><p className="eyebrow">Visão geral</p><h2>Dashboard</h2></div><button className="ghost-btn">Exportar resumo</button></div><div className="stats-grid"><StatCard label="Usuários ativos" value={String(kpis.users)} delta="persistidos no banco" /><StatCard label="Relatórios ativos" value={String(kpis.reports)} delta="maiores quedas pronto" /><StatCard label="Agendamentos" value={String(kpis.schedules)} delta="persistidos no banco" /><StatCard label="Histórico" value={String(kpis.historyItems)} delta="execuções registradas" /></div><div className="content-grid"><div className="panel large"><h3>Pipeline operacional</h3><div className="bars"><div><span>Captura</span><strong style={{ width: '84%' }} /></div><div><span>Processamento</span><strong style={{ width: '62%' }} /></div><div><span>Envio</span><strong style={{ width: '91%' }} /></div><div><span>Validação</span><strong style={{ width: '73%' }} /></div></div></div><div className="panel"><h3>Próximas ações</h3><ul className="list"><li>Conectar envio automático</li><li>Exportar Excel</li><li>Refinar projeção</li></ul></div></div></section>
}
...
function ReportsPage() {
  ...
  async function loadReport(date = referenceDate, nextVendedor = vendedor, nextSupervisor = supervisor, nextTop = top) {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/maiores-quedas?${query.toString()}`)
  ...
  async function createSchedule() {
    ...
    const json = await response.json()
    if (!response.ok) { setScheduleMessage(json.message || 'Erro ao criar agendamento'); return }
    setScheduleMessage(`Agendamento criado #${json.id}`)
  }

  useEffect(() => { loadFilters().catch(() => undefined); loadReport(today, '', '', 30).catch(() => undefined) }, [])

  return <section className="page"><div className="page-header"><div><p className="eyebrow">Relatório real</p><h2>Maiores quedas</h2></div><button className="ghost-btn">Motor em produção</button></div><div className="panel filters-panel"><div className="filter-grid"><label>Data de referência<input type="date" value={referenceDate} onChange={(e) => setReferenceDate(e.target.value)} /></label><label>Supervisor<select value={supervisor} onChange={(e) => setSupervisor(e.target.value)}><option value="">Todos</option>{filters.supervisores.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>RCA<select value={vendedor} onChange={(e) => setVendedor(e.target.value)}><option value="">Todos</option>{filters.vendedores.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>Top N<input type="number" min={5} max={200} value={top} onChange={(e) => setTop(Number(e.target.value) || 30)} /></label></div><div className="actions-row top-space"><button onClick={() => loadReport()}> {loading ? 'Atualizando...' : 'Atualizar relatório'} </button></div>{data ? <p className="muted small-text">Período atual: {data.periods.current_start} até {data.periods.current_end} ({data.periods.current_days} dias úteis + sábados) • Período anterior: {data.periods.previous_start} até {data.periods.previous_end} ({data.periods.previous_days} dias úteis + sábados)</p> : null}{error ? <p className="error-text">{error}</p> : null}</div>{data ? <div className="stats-grid report-stats"><StatCard label="Clientes em queda" value={String(data.summary.clientesEmQueda)} delta="ranking atual" /><StatCard label="Perda acumulada" value={brl.format(data.summary.perdaAcumulada)} delta="top retornado" /><StatCard label="Mês atual" value={brl.format(data.summary.vendaMesAtual)} delta="pedidos posição F" /><StatCard label="Mês passado" value={brl.format(data.summary.vendaMesPassado)} delta="recorte equivalente" /></div> : null}<div className="panel schedule-panel"><h3>Agendar envio diário</h3><div className="filter-grid"><label>Nome da regra<input value={scheduleForm.ruleName} onChange={(e) => setScheduleForm({ ...scheduleForm, ruleName: e.target.value })} /></label><label>Tipo de alvo<select value={scheduleForm.targetType} onChange={(e) => setScheduleForm({ ...scheduleForm, targetType: e.target.value })}><option value="vendedor">vendedor</option><option value="supervisor">supervisor</option><option value="gerente">gerente</option><option value="grupo_contato">grupo_contato</option></select></label><label>Alvo<input value={scheduleForm.targetId} onChange={(e) => setScheduleForm({ ...scheduleForm, targetId: e.target.value })} placeholder="Ex.: JÚNIOR FOBA" /></label><label>Hora<input type="time" value={scheduleForm.sendTime} onChange={(e) => setScheduleForm({ ...scheduleForm, sendTime: e.target.value })} /></label><label>Canal<select value={scheduleForm.channel} onChange={(e) => setScheduleForm({ ...scheduleForm, channel: e.target.value })}><option value="whatsapp">whatsapp</option><option value="email">email</option></select></label></div><div className="actions-row top-space"><button onClick={createSchedule}>Criar agendamento</button></div>{scheduleMessage ? <p className="info-text">{scheduleMessage}</p> : null}</div><div className="panel table-panel"><table><thead><tr><th>Cód.</th><th>Cliente</th><th>Cidade</th><th>RCA</th><th>Supervisor</th><th>Telefone</th><th>Mês passado</th><th>Mês atual</th><th>Projeção</th><th>Perda</th><th>Queda %</th><th>Tendência</th></tr></thead><tbody>{data?.items.map((item) => <tr key={`${item.cod_cliente}-${item.rca}`}><td>{item.cod_cliente}</td><td>{item.cliente}</td><td>{item.cidade}</td><td>{item.rca}</td><td>{item.supervisor}</td><td>{item.telefone || '-'}</td><td>{brl.format(item.mes_passado)}</td><td>{brl.format(item.mes_atual)}</td><td>{brl.format(item.projecao_mes)}</td><td className={item.perda_valor < 0 ? 'negative' : 'positive'}>{brl.format(item.perda_valor)}</td><td className={item.perda_percentual < 0 ? 'negative' : 'positive'}>{item.perda_percentual}%</td><td><span className={`tag ${item.tendencia === 'queda' ? 'danger' : item.tendencia === 'alta' ? 'success' : 'warning'}`}>{item.tendencia}</span></td></tr>)}</tbody></table></div></section>
}

function SchedulesPage() {
  const [items, setItems] = useState<Schedule[]>([])
  useEffect(() => { fetch('/api/schedules').then((r) => r.json()).then(setItems).catch(() => undefined) }, [])
  return <section className="page"><div className="page-header"><div><p className="eyebrow">Rotinas</p><h2>Agendamentos</h2></div><button className="ghost-btn">Atualizar</button></div><div className="panel table-panel"><table><thead><tr><th>Regra</th><th>Relatório</th><th>Alvo</th><th>Hora</th><th>Canal</th><th>Status</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.rule_name}</td><td>{item.report_type_code}</td><td>{item.target_type}: {item.target_id}</td><td>{String(item.send_time).slice(0,5)}</td><td>{item.channel}</td><td><span className={`tag ${item.active ? 'success' : 'warning'}`}>{item.active ? 'Ativo' : 'Inativo'}</span></td></tr>)}</tbody></table></div></section>
}

function HistoryPage() {
  const [items, setItems] = useState<HistoryItem[]>([])
  useEffect(() => { fetch('/api/history').then((r) => r.json()).then(setItems).catch(() => undefined) }, [])
  return <section className="page"><div className="page-header"><div><p className="eyebrow">Auditoria</p><h2>Histórico</h2></div><button className="ghost-btn">Atualizar</button></div><div className="panel table-panel"><table><thead><tr><th>ID</th><th>Regra</th><th>Relatório</th><th>Alvo</th><th>Status</th><th>Criado em</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{item.id}</td><td>{item.rule_name}</td><td>{item.report_type_code}</td><td>{item.target_type}: {item.target_id}</td><td>{item.status}</td><td>{item.created_at}</td></tr>)}</tbody></table></div></section>
}
...
function UsersPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); setMessage(''); try { const payload = { ...form, active: Boolean(form.active) }; const response = await fetch(editingId ? `/api/users/${editingId}` : '/api/users', { method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const data = await response.json(); if (!response.ok) throw new Error(data.message || 'Falha ao salvar usuário'); await loadUsers(); setMessage(editingId ? 'Usuário atualizado' : 'Usuário criado'); resetForm() } catch (err) { setMessage(err instanceof Error ? err.message : 'Erro ao salvar usuário') } finally { setLoading(false) } }
  async function handleDelete(id: number) { if (!confirm('Remover este usuário?')) return; const response = await fetch(`/api/users/${id}`, { method: 'DELETE' }); const data = await response.json(); if (!response.ok) { setMessage(data.message || 'Erro ao remover usuário'); return } await loadUsers(); setMessage('Usuário removido'); if (editingId === id) resetForm() }
